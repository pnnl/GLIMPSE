...
const Graph = ({ dataToVis, theme, isGlm, isCim, modelNumber, setSearchReqs }) => {
  ...
  const addToWatchList = (id, elementType) => {
    if (elementType === 'node') {
      watchList.nodes.push(id);
    } else if (elementType === 'edge') {
      watchList.edges.push(id);
    }

    console.log(`Added ${elementType} : ${id} to watch list`);
    ...
    const initializeGraph = async () => {
      ...
      network.on('oncontext', (params) => {
        ...
        if (contextEdgeID && !contextNodeID) {
          if (watchList.edges.includes(contextEdgeID)) {
            setContextMenuData({
          ...
          if (watchList.nodes.includes(nodeData.id)) {
            setContextMenuData({
              nodeID: nodeData.id,
              watch: true
            });
