...
const Graph = ({ dataToVis, theme, isGlm, isCim, setSearchReqs }) => {
    ...
    if (isCim) {
      ...
      const newCNode = {
        ...
        attributes: {
          mRID: NEW_C_NODE_ID,
          name: `c_node:${NEW_C_NODE_ID}`,
          terminals: [TERMINAL_1_ID, NEW_NODE_ID].join(',')
        }
      };
      ...
      let edgeID = `line:${NEW_C_NODE_ID}-${NEW_NODE_ID}`;
      let newEdge = {
        id: edgeID,
        from: NEW_C_NODE_ID,
        to: NEW_NODE_ID,
      ...
      terminal1.title = getTitle({ ObjectType: 'terminal', ...terminal1.attributes });
      newNodes.push(terminal1);

      // connect terminal 1 with new connectivity node
      edgeID = `line:${NEW_C_NODE_ID}-${TERMINAL_1_ID}`;
      newEdge = {
        id: edgeID,
        from: NEW_C_NODE_ID,
        to: TERMINAL_1_ID,
      ...
      newEdge.title = getTitle({ objectType: 'line', ...newEdge.attributes });
      newEdges.push(newEdge);
      ...
      terminal2.title = getTitle({ objectType: 'terminal', ...terminal2.attributes });
      newNodes.push(terminal2);

      // connect terminal #2 to existing connectivity node
      edgeID = `line:${connectTo}-${TERMINAL_2_ID}`;
      newEdge = {
        id: edgeID,
        from: connectTo,
        to: TERMINAL_2_ID,
      ...
      edgeID = `${edgeTypes[edgeType]}:${TERMINAL_2_ID}-${TERMINAL_1_ID}`;
      newEdge = {
        id: edgeID,
        from: TERMINAL_2_ID,
        to: TERMINAL_1_ID,
  ...
  useEffect(() => {
    ...
    removeListenerArr.push(
      window.glimpseAPI.onExtract(() => Export(graphData, isGlm, isCim, dataToVis, newCIMobjs))
    );
    removeListenerArr.push(
