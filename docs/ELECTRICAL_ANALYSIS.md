...
| Where | What |
| --- | --- |
| **Toolbar ⚠ button** (or `v`) | Toggles *violation mode*. Enabled only while a simulation is `running` or `paused`. |
...
| **Node hover card** | A bold, color-coded *vitals* block — per-phase p.u. and volts — above the model attributes. |
| **Edge width & flow dots** | Thickness and dot speed scale with loading, in *any* mode — not just violation mode. |
...
re-aggregated from its *full* per-phase map rather than from the current
...
| Answers | "what voltage *should* this bus be at?" | "how much can this conductor *carry*?" |
...
This is also why the PNV block in `handleSimulationOutput` runs *before* the VA
...
- **CIM / GridAPPS-D** models don't expose a numeric base: `cimhelper._add_attributes`
  stringifies `BaseVoltage` to its *name*, not its value.
...
- **Deeply depressed buses can mis-resolve their base.** Adjacent distribution
  ...
  snapper takes the *closest* candidate rather than requiring an unambiguous
...
- **Loading needs a rating *and* a voltage.** `normal_limit` is in amperes, so
  ...
  note and classifies as *No data* — never as "normal", which would imply a
...
```bash
npm run test:js          # both suites
npm run test:electrical  # p.u. / loading math
npm run test:hover       # hover-card attribute selection
```
...
originally resolved *nothing* in the 7–8 kV band, and the hover filter rendered
